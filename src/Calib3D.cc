...
{
    Local<Object> matrixWrap = Nan::New(Matrix::constructor)->GetFunction()->NewInstance();
...
inline std::vector<cv::Point2f> points2fFromArray(Handle<Value> array)
{
    ...
    if(array->IsArray())
    {
        ...
        for(unsigned int i = 0; i < pointsArray->Length(); i++)
        {
            ...
            points.push_back(cv::Point2f(pt->Get(Nan::New<String>("x").ToLocalChecked())->ToNumber()->Value(),
...
inline std::vector<cv::Point3f> points3fFromArray(Handle<Value> array)
{
    ...
    if(array->IsArray()) {
        ...
        {
            Local<Object> pt = pointsArray->Get(i)->ToObject();
            points.push_back(cv::Point3f(pt->Get(Nan::New<String>("x").ToLocalChecked())->ToNumber()->Value(),
                                         pt->Get(Nan::New<String>("y").ToLocalChecked())->ToNumber()->Value(),
...
NAN_METHOD(Calib3D::CalibrateCamera)
{
    ...
    try {
        ...
        std::vector<std::vector<cv::Point3f> > objectPoints = points3fFromArrayOfArrays(info[0]);

        // Arg 1, the image points, another array of arrays
        std::vector<std::vector<cv::Point2f> > imagePoints = points2fFromArrayOfArrays(info[1]);
...
NAN_METHOD(Calib3D::StereoCalibrate)
{
    ...
    try {
        ...
        std::vector<std::vector<cv::Point2f> > imagePoints1 = points2fFromArrayOfArrays(info[1]);

        // Arg 2, the image points2, another array of arrays =(
        std::vector<std::vector<cv::Point2f> > imagePoints2 = points2fFromArrayOfArrays(info[2]);
