...
Matrix::Matrix(): Nan::ObjectWrap() {
	mat = cv::Mat();
}


Matrix::Matrix(int rows, int cols): Nan::ObjectWrap() {
    mat = cv::Mat(rows, cols, CV_32FC3);
}

Matrix::Matrix(int rows, int cols, int type): Nan::ObjectWrap() {
    mat = cv::Mat(rows, cols, type);
}

Matrix::Matrix(cv::Mat m, cv::Rect roi): Nan::ObjectWrap() {
	mat = cv::Mat(m, roi);
}
...
NAN_METHOD(Matrix::Normalize) {
	...
	if (info[2]->IsNumber()){
		...
		if (
			...
			Nan::ThrowTypeError("type value must be NORM_INF=1, NORM_L1=2, NORM_L2=4, NORM_L2SQR=5, NORM_HAMMING=6, NORM_HAMMING2=7, NORM_TYPE_MASK=7, NORM_RELATIVE=8, NORM_MINMAX=32 ");
		}
	}
	int dtype = -1;
	if (info[3]->IsNumber()){
...
NAN_METHOD(Matrix::ToBuffer){
    ...
    if ((info.Length() > 0) && (info[0]->IsObject())) {
        ...
        if (options->Has(Nan::New<String>("ext").ToLocalChecked())) {
            v8::String::Utf8Value str ( options->Get(Nan::New<String>("ext").ToLocalChecked())->ToString() );
            optExt = *str;
            ext = (const char *) optExt.c_str();
        }
        if (options->Has(Nan::New<String>("jpegQuality").ToLocalChecked())) {
            int compression = options->Get(Nan::New<String>("jpegQuality").ToLocalChecked())->IntegerValue();
            params.push_back(CV_IMWRITE_JPEG_QUALITY);
            params.push_back(compression);
        }
        if (options->Has(Nan::New<String>("pngCompression").ToLocalChecked())) {
            int compression = options->Get(Nan::New<String>("pngCompression").ToLocalChecked())->IntegerValue();
  ...
  memcpy(data, &vec[0], vec.size());
	...
	v8::Local<v8::Object> globalObj = Nan::GetCurrentContext()->Global();
	v8::Local<v8::Function> bufferConstructor = v8::Local<v8::Function>::Cast(globalObj->Get(Nan::New<String>("Buffer").ToLocalChecked()));
	v8::Handle<v8::Value> constructorArgs[3] = {buf, Nan::New<v8::Integer>((unsigned)vec.size()), Nan::New<v8::Integer>(0)};
	v8::Local<v8::Object> actualBuffer = bufferConstructor->NewInstance(3, constructorArgs);
...
class AsyncToBufferWorker : public Nan::AsyncWorker {
 public:
  AsyncToBufferWorker(Nan::Callback *callback, Matrix* matrix, string ext, vector<int> params )
    : Nan::AsyncWorker(callback), matrix(matrix), ext(ext), params(params) {}
  ~AsyncToBufferWorker() {}
...
NAN_METHOD(Matrix::Ellipse){
	...
	if(info[0]->IsObject()) {
		...
		if (options->Has(Nan::New<String>("center").ToLocalChecked())) {
		  Local<Object> center = options->Get(Nan::New<String>("center").ToLocalChecked())->ToObject();
		...
		if (options->Has(Nan::New<String>("color").ToLocalChecked())) {
			Local<Object> objColor = options->Get(Nan::New<String>("color").ToLocalChecked())->ToObject();
...
class AsyncSaveWorker : public Nan::AsyncWorker {
 public:
  AsyncSaveWorker(Nan::Callback *callback, Matrix* matrix, char* filename)
    : Nan::AsyncWorker(callback), matrix(matrix), filename(filename) {}
  ~AsyncSaveWorker() {}
...
NAN_METHOD(Matrix::Flip) {
	...
	if ( info.Length() < 1 || !info[0]->IsInt32() ) {
		Nan::ThrowTypeError("Flip requires an integer flipCode argument (0 = X axis, positive = Y axis, negative = both axis)");
	}
...
NAN_METHOD(Matrix::Rotate){
  ...
  int x = info[1]->IsUndefined() ? round(self->mat.size().width / 2) : info[1]->Uint32Value();
  int y = info[1]->IsUndefined() ? round(self->mat.size().height / 2) : info[2]->Uint32Value();
...
NAN_METHOD(Matrix::Split) {
  ...
  for (unsigned int i = 0; i < size; i++) {
      Local<Object> matObject = Nan::New(Matrix::constructor)->GetFunction()->NewInstance();
...
NAN_METHOD(Matrix::FloodFill){
	...
	int  ret = cv::floodFill(self->mat, setPoint(obj->Get(Nan::New<String>("seedPoint").ToLocalChecked())->ToObject())
			, setColor(obj->Get(Nan::New<String>("newColor").ToLocalChecked())->ToObject())
			, obj->Get(Nan::New<String>("rect").ToLocalChecked())->IsUndefined() ? 0 : setRect(obj->Get(Nan::New<String>("rect").ToLocalChecked())->ToObject(), rect)
			, setColor(obj->Get(Nan::New<String>("loDiff").ToLocalChecked())->ToObject())
			, setColor(obj->Get(Nan::New<String>("upDiff").ToLocalChecked())->ToObject())
			, 4 );
...
NAN_METHOD(Matrix::TemplateMatches){
  ...
  bool filter_min_probability = (info.Length() >= 1) ? info[0]->IsNumber() : false;
  bool filter_max_probability = (info.Length() >= 2) ? info[1]->IsNumber() : false;
